...
class Storage{
  ...
  static Future<void> load() async{
    ...
    await database.execute("CREATE TABLE IF NOT EXISTS messages (id INTEGER, message TEXT, sender VARCHAR(15), group_uuid CHAR(36), uuid CHAR(36) UNIQUE, seen INTEGER);");
    isLoaded = true;
  }
  static Future<List<MessageModel>> getMessages(String groupUUID, int fromMessageID, int count) async{
    ...
    return Future.value(query.map((e) => MessageModel(e["message"], e["sender"], e["group_uuid"], e["uuid"], e["id"], e["seen"] == 0 ? false : true)).toList());
  }
  static void addMessage(MessageModel msg) async{
    int messages = await getMessagesCount(msg.groupUUID);
    await database.insert("messages", {"message": msg.message, "sender": msg.sender, "group_uuid": msg.groupUUID, "uuid": msg.uuid, "id": messages+1, "seen": msg.seen ? 1 : 0}, conflictAlgorithm: ConflictAlgorithm.ignore);
  }
